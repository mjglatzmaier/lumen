...
#define NUM_TEST_JOBS 4
#define WORKER_COUNT 2  // Must match the definition in job_scheduler.c
...
static bool test_performance() {
    ...
    int** allocated_data = allocator->alloc(allocator, NUM_TEST_JOBS * sizeof(int*), 16);
    if (!allocated_data) {
        printf("Failed to allocate job data array!\n");
        return false;
    }

    for (int i = 0; i < NUM_TEST_JOBS; i++) {
        allocated_data[i] = allocator->alloc(allocator, sizeof(int), 16);
        if (!allocated_data[i]) {
            printf("Failed to allocate job data for job %d!\n", i);
            return false;
        }
        *(int *)allocated_data[i] = i % WORKER_COUNT;
        job_scheduler_submit(test_job_function, allocated_data[i]);
    }
    // Free all allocated job data **after** job execution is complete
    for (int i = 0; i < NUM_TEST_JOBS; i++) {
        allocator->free(allocator, allocated_data[i]);
    }
    allocator->free(allocator, allocated_data);
    job_scheduler_shutdown();
