...
static void *lum_pool_allocator_alloc(lum_allocator *allocator, size_t unused_size,
                                      size_t unused_align)
{
    (void) unused_size;
    (void) unused_align;
...
lum_allocator *lum_create_pool_allocator(size_t object_size, size_t object_count)
{
    ...
    lum_pool_allocator *pool =
        allocator->alloc(allocator, sizeof(lum_pool_allocator), _Alignof(lum_pool_allocator));
    if (!pool)
    {
    ...
    pool->free_list =
        (void **) allocator->alloc(allocator, sizeof(void *) * object_count, _Alignof(void *));
    if (!pool->free_list)
    {
