...
lum_allocator *lum_create_frame_allocator(size_t size)
{
    ...
    lum_frame_allocator *frame =
        allocator->alloc(allocator, sizeof(lum_frame_allocator), _Alignof(lum_frame_allocator));
    ...
    assert(((uintptr_t) frame->buffer & (FRAME_ALIGN - 1)) ==
           0); //&& "Frame buffer is misaligned!");
    return allocator;
}
