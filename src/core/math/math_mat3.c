...
bool lum_mat3_inverse(lum_mat3* out, const lum_mat3* m) {
    ...
    if (fabs(det) < 1e-6f) return false; // Singular matrix, no inverse
...
void lum_mat3_scale(lum_mat3* out, const lum_mat3* m, float scalar) {
    out->m[0] = m->m[0] * scalar;  out->m[1] = m->m[1] * scalar;  out->m[2] = m->m[2] * scalar;
    out->m[3] = m->m[3] * scalar;  out->m[4] = m->m[4] * scalar;  out->m[5] = m->m[5] * scalar;
    out->m[6] = m->m[6] * scalar;  out->m[7] = m->m[7] * scalar;  out->m[8] = m->m[8] * scalar;
}

// Matrix Addition: out = A + B
void lum_mat3_add(lum_mat3* out, const lum_mat3* A, const lum_mat3* B) {
    out->m[0] = A->m[0] + B->m[0];  out->m[1] = A->m[1] + B->m[1];  out->m[2] = A->m[2] + B->m[2];
    out->m[3] = A->m[3] + B->m[3];  out->m[4] = A->m[4] + B->m[4];  out->m[5] = A->m[5] + B->m[5];
    out->m[6] = A->m[6] + B->m[6];  out->m[7] = A->m[7] + B->m[7];  out->m[8] = A->m[8] + B->m[8];
}
